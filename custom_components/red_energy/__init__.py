...
async def async_migrate_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Handle Home Assistant's built-in config entry migration.

    Delegates to the integration's internal migrator to keep a single migration path.
    """
    _LOGGER.debug("HA requested migration for entry %s (version %s)", entry.entry_id, entry.version)
    try:
        ...
        _LOGGER.error("Config entry migration failed: %s", err)
        return False
